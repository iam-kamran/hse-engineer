import { getCertModule, getCertModules } from "@/lib/data-loaders";
...
export function generateMetadata({
  ...
  params: { cert: string; module: string };
}): Metadata {
  const mod = getCertModule(params.cert, params.module);
  if (!mod) return { title: "Module | HSE Engineer" };
  return {
...
export default function CertModulePage({
  params,
}: {
  params: { cert: string; module: string };
}) {
    ...
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-slate-500 mb-6">
          This module is being written. Check back soon!
        </p>
        <Link
          href={`/certifications/${params.cert}`}
          className="text-green-600 hover:text-green-700 font-medium"
        >
          ← Back to {params.cert.toUpperCase()}
        </Link>
      </div>
    );
  }

  const all = getCertModules(params.cert);
  const idx = all.findIndex((m) => m.id === mod.id);
  const prev = all[idx - 1];
  const next = all[idx + 1];

  const certLabel = params.cert.replace(/-/g, " ").toUpperCase();
    ...
    <article>
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-sm text-slate-400">~{mod.estimatedReadTime} min read</span>
        </div>
      ...
      <nav className="mt-8 grid grid-cols-2 gap-4">
        {prev && (
          <Link
            href={`/certifications/${params.cert}/${prev.id.split("/").pop()}`}
        ...
        {next && (
          <Link
            href={`/certifications/${params.cert}/${next.id.split("/").pop()}`}
